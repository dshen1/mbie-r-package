...
growth <- plyr::ddply(RTEs, .(Territorial_Authority, Region), summarise,
                growth=CAGR(sum(Spend[YEMar==2013]) / sum(Spend[YEMar==2009]), 4))

# Merge with the absolute size of tourism in 2013
RTE2 <- merge(plyr::ddply(subset(RTEs, YEMar==2013), .(Territorial_Authority, Region), summarise,
                    Spend=sum(Spend)),
              growth)
...
data(IVStrips)
tmp <- plyr::ddply(IVStrips, .(YearQuarter, CountryGroup(COPRTop5)), summarise, Total= sum(SmoothTotalSpend * FinalWeight * Adjusted_Factor))
...
dom <- plyr::ddply(subset(RTEs, Type == "Domestic"), .(RTO), summarise,
  Growth = CAGR(sum(Spend[YEMar==2013]) / sum(Spend[YEMar==2009]), 4),
